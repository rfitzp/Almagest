# Script to determine new moons in given year
...
def Get_Lambda_Moon (t):
    ...
    bS, bSt, bSa = Get_R_Moon (t)

    if abs (betaM) < bSt or abs (betaM) < bSa:
        mag = (bS - bSa) /(bS - bSt)
    else:
...
def Get_DMS (t):
    ...
    lamS, SM, rhoM                = Get_Lambda_Sun  (t)
    lamM, bsyz, bS, bSt, bSa, mag = Get_Lambda_Moon (t)
...
for i in range (1, 14):
    ...
    lamM, bsyz, bS, bSt, bSa, mag  = Get_Lambda_Moon (jd)
    ...
    if c == ' ':    
        ...
        print ("%02d/%02d/%4d &  %02d:%02d & %.1f & %.1f & %.1f & %05.1f & %s & %.2f\\\\" % (day, month, year, hour, minute, bS, bSt, bSa, abs (bsyz), c, mag), file=f)
