...
(because both the Moon and the shadow are the same distance from the Earth).
 ...
 Note that the Sun has negligible parallax, because it is much further from the Earth than the Moon. Taking parallactic shifts into account, a {\em total solar eclipse}, in which the Sun is totally obscured by the Moon,
...
of 1992 AD, calculated using the method described at the beginning of
...
Equations~(\ref{est})--(\ref{esp}). It can be seen that
...
Equations~(\ref{elp}) and (\ref{elt}), respectively. It can be seen that
...
and the criteria (\ref{elt})--(\ref{eps}).
